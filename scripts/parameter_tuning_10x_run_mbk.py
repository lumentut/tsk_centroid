...
def ga_mbkmeans_cluster_optimize(
    ...
    def fitness_fn(ga_instance, solution, solution_idx):
        try:
            ...
            pipeline.fit(
                ...
                predictor__mfs__cluster__init_method=solution[4],
                ...
                predictor__rules__cluster__init_method=solution[4],
                predictor__mf_type=MFType2.GAUSSIAN,
                predictor__mf__builder__uncertainty_factor=solution[5],
                predictor__mf__builder__min_std_ratio=solution[6],
            ...
            statistics.append(
                {
                    ...
                    "init_method": {solution[4]},
                    "uncertainty_factor": {solution[5]},
                    "min_std_ratio": {solution[6]},
                }
            )
    ...
    gene_space = [
        ...
        [
            max(100, n_samples // 20),
            max(256, n_samples // 10),
            max(512, n_samples // 8),
            max(1024, n_samples // 4),
            min(2048, n_samples // 2),
        ],
        [50, 100, 150, 200, 300],  # max_iter
        {"low": 1e-5, "high": 1e-2},  # tol
        [5, 10, 15, 20],  # max_no_improvement
        {"low": 0.0, "high": 1.0},  # init method
