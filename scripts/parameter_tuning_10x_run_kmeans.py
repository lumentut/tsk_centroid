...
logging.info("=======================================================")
logging.info("K-Means Clustering Hyperparameter Tuning Script")
logging.info("========================================================\n")
...
def ga_kmeans_cluster_optimize(
    ...
    def fitness_fn(ga_instance, solution, solution_idx):
        try:
            ...
            pipeline.fit(
                ...
                predictor__mfs__cluster__n_init=int(solution[0]),
                predictor__mfs__cluster__max_iter=int(solution[1]),
                predictor__mfs__cluster__tol=solution[2],
                predictor__mfs__cluster__init_method=solution[3],
                predictor__rules__cluster__n_init=int(solution[0]),
                predictor__rules__cluster__max_iter=int(solution[1]),
                predictor__rules__cluster__tol=solution[2],
                predictor__rules__cluster__init_method=solution[3],
    ...
    gene_space = [
        [5, 10, 15, 20],  # n_init (discrete)
        [100, 200, 300, 400, 500],  # max_iter (discrete)
        {"low": 1e-5, "high": 1e-2},  # tol (continuous)
        {"low": 0.0, "high": 1.0},  # init method (0-0.5: k-means++, 0.5-1: random)
