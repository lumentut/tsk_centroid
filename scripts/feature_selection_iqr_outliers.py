...
from src.pipelines.transformers import FeatureScaler, OutlierScoreSelector
...
logging.info("========================================")
logging.info("Feature Selection (Outlier-based) Comparison Test")
logging.info("========================================\n")
...
def feature_selection_iqr(sheet_name: str, iteration_num: int):
    ...
    data = [
        item
        for item in detect_outliers(df=dataset.train_df)["iqr"][:-2]
        if list(item.keys())[0] != target_column
    ]
    ...
    def predict(feature_name: str, threshold: float):
        ...
        pipeline = Pipeline(
            steps=[
                ...
                (
                    "feature_selection",
                    OutlierScoreSelector(target=target_column, method="iqr"),
                ),
    ...
    for _, item in enumerate(data):
        # Get the key (MQ name)
        mq_name = list(item.keys())[0]
        mq_value = list(item.values())[0]
        cumulative_list.append(mq_name)
        removed_features = cumulative_list.copy()
        keep_features = [col for col in train_df.columns if col not in removed_features]
        logging.info(f"Processing features: {keep_features} ...")
        predict(feature_name=mq_name, threshold=mq_value)


start = time.time()
for worksheet in WorkSheet:
    for i in range(1000):
        feature_selection_iqr(sheet_name=worksheet.value, iteration_num=i)

end = time.time()
elapsed = end - start

logging.info(
    f"Running feature selection iqr test... Elapsed time: {timedelta(seconds=elapsed)}"
)
