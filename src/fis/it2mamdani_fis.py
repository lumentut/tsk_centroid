...
from pyit2fls import IT2Mamdani
...
class IT2MamdaniFIS(BaseFIS):
    ...
    def create_fis(self) -> IT2MamdaniFIS:
        self.fis = IT2Mamdani(
            t_norm=t_norm_fn(self.t_norm),
            s_norm=s_norm_fn(self.s_norm),
            method="Centroid",
            algorithm="EKM",
        )
        return self

    def add_input_variables(self) -> IT2MamdaniFIS:
        for var in self.input_variables:
            self.fis.add_input_variable(var["name"])
        return self

    def add_output_variables(self) -> IT2MamdaniFIS:
        for var in self.output_variables:
            self.fis.add_output_variable(var["name"])
        return self

    def add_rules(self) -> IT2MamdaniFIS:
        for rule in self.rules:
            self.fis.add_rule(rule["antecedents"], rule["consequents"])
        return self

    def build(self) -> IT2Mamdani:
        return (
            self.create_fis()
            .add_input_variables()
