    ...
    def fit(self, df: pd.DataFrame, **kwargs) -> IT2TskPredictor:
        ...
        self.io_vars_ = IOVariable(
            ...
            mf_builder=MFFactory.create_mf_builder(
                name=kwargs.get("mf_type", MFType2.GAUSSIAN), **kwargs
            ),
        )
        ...
        for _, row in input_df.iterrows():
            ...
            try:
                ...
                if isinstance(output_dict, dict) and self.target in output_dict:
                    predicted_value = output_dict[self.target]
                else:
                ...
                predictions.append(min(max(0, predicted_value), 1))
